...
export class QueryMananger{
    ...
    async getRecordsByFields(sobjectName, fields, offset, condition, extfields, pageLength){
        ...
        if (fieldNames){
            let idFieldDesc = sobjectDescribe.fields.find(e=>e.name=='Id');
            acQuery=`select ${fieldNames} from ${sobjectDescribe.name} ${conditionStr} ${idFieldDesc.sortable?'order by Id':''} ${offset==0||offset>2000?'':'limit ' +offset}`;
            acQuery = '/services/data/v56.0/query/?q=' + encodeURIComponent(acQuery);
        }else{
            acQuery = "/services/data/v" + apiVersion + "/sobjects/" + sobjectDescribe.name + "/"+value;
        }
