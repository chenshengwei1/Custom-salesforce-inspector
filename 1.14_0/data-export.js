...
window.globalData={sfConn,apiVersion,Enumerable,DescribeInfo,copyToClipboard,initScrollTable}
...
class Model {
  ...
  didUpdate(cb) {
    ...
    window.autocompleteResults&&window.autocompleteResults(this.autocompleteResults.results);
  }
  /**
    ...
    function batchHandler(batch) {
      return batch.catch(err => {
       ...
       if (window.filterRecords){
        data.records = window.filterRecords(data.records);
       }
      ...
      h("div", {className: "area"},
        ...
        h("label", {},
          h("select", {value: JSON.stringify(model.selectedHistoryEntry), onChange: this.onSelectHistoryEntry, className: "query-history"},
            ...
            model.queryHistory.list.map(q => h("option", {key: JSON.stringify(q), value: JSON.stringify(q), title: JSON.stringify(q)}, q.query.substring(0, 300)))
          ),
        ...
        h("label", {},
          h("select", {value: JSON.stringify(model.selectedSavedEntry), onChange: this.onSelectSavedEntry, className: "query-history"},
            ...
            model.savedHistory.list.map(q => h("option", {key: JSON.stringify(q), value: JSON.stringify(q), title: JSON.stringify(q)}, q.query.substring(0, 300)))
          ),
