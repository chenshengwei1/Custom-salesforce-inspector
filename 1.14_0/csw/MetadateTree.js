   ...
   createHead(rootId){
    ...
    if (!this.init ){
       ...
       <div class="searchresult">
           <div class="totalbar" style="width: 100%;"><span>Total Records : </span><span class="recordsnumber">0</span></div>
           <div class="totalbar" style="width: 100%;">
       ...
       $('#metadataDetails .content').on('click', 'button[name].meta', (event)=>{
           ...
           let update = (data, isNew)=>{
               ...
               let content = Object.keys(data||{}).map(e=>{
                    // <xmp1>${this.wapper(data[e], e)}</xmp1>
                    let id = this.tree.getUuid();
                    ids.push(id);
                   return ` <h1>${e}</h1>
                   <p class="code-example2"><div class="code " id="${id}">${data[e]}</div> </p>`
               }).join('');
           ...
           let metadataFile = this.metadataDetails[xName].find(e => e.fullName == fileName);
